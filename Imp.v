...
Fixpoint optimize_0plus_b (b : bexp) : bexp :=
  ...
  | BNeq b1 b2 => BNeq (optimize_0plus b1) (optimize_0plus b2)
...
Proof. 
  ...
  - intros H; induction H; simpl;
    try (reflexivity);
    try (rewrite !aevalR_iff_aeval in *; subst; reflexivity);
    try (rewrite IHbevalR; reflexivity);
    try (rewrite IHbevalR1, IHbevalR2; reflexivity).
  - generalize dependent bv.
  induction b; intros bv H; simpl in H; subst. (*constructor; try apply aevalR_iff_aeval; *)
  + constructor.
  + constructor.
  + constructor; apply aevalR_iff_aeval; reflexivity.
  + constructor; apply aevalR_iff_aeval; reflexivity.
  + constructor; apply aevalR_iff_aeval; reflexivity.
  + constructor; apply aevalR_iff_aeval; reflexivity.
...
| nw_Skip : no_whilesR <{ skip }>
  ...
  | nw_Seq : forall c1 c2 (H1: no_whilesR c1) (H2: no_whilesR c2), no_whilesR <{ c1; c2 }>
  | nw_If : forall b c1 c2 (H1: no_whilesR c1) (H2: no_whilesR c2), no_whilesR <{ if b then c1 else c2 end }>
.
...
Proof.
  intros. generalize dependent st. induction H.
    ...
    - intros. destruct (IHno_whilesR1 st) as [st' H1'].
    destruct (IHno_whilesR2 st') as [st'' H2']. exists st''.
...
Proof.
  intros st p1.
  induction p1 as [| instr p1' IHp1]; intros p2 stack.
  - reflexivity.
  - simpl.
    destruct instr;
    try reflexivity;
    destruct stack as [| n1 [| n2 stack']]; try reflexivity;
...
| E_Seq1 : forall c1 c2 st st', st  =[ c1 ]=> st' / SBreak  -> st  =[ c1 ; c2 ]=> st' / SBreak
| E_Seq2 : forall c1 c2 st st' st'' s, st  =[ c1 ]=> st' / SContinue  -> st' =[ c2 ]=> st'' / s  -> st  =[ c1 ; c2 ]=> st'' / s
| E_IfTrue : forall st st' b c1 c2 s, beval st b = true -> st =[ c1 ]=> st' / s -> st =[ if b then c1 else c2 end]=> st' / s
| E_IfFalse : forall st st' b c1 c2 s, beval st b = false -> st =[ c2 ]=> st' / s -> st =[ if b then c1 else c2 end]=> st' / s
| E_WhileFalse : forall b st c, beval st b = false -> st =[ while b do c end ]=> st / SContinue
| E_WhileTrue1 : forall st st' b c, beval st b = true -> st  =[ c ]=> st' / SBreak -> st  =[ while b do c end ]=> st' / SContinue
| E_WhileTrue2 : forall st st' st'' b c, beval st b = true -> st  =[ c ]=> st' / SContinue -> st' =[ while b do c end ]=> st'' / SContinue -> 
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros. inversion H0; subst; apply E_WhileTrue1;
...
Theorem seq_stops_on_break : forall c1 c2 st st',
  ...
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
intros. apply E_Seq1; assumption. Qed.
  (** [] *)
